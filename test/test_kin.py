...
class IKTestCase(unittest.TestCase):
    ...
    def test_01(self):
        q = self.kin.ik(2 * Kinematics.L_SEGMENT + Kinematics.L_GRIPPER, 0, Kinematics.Z_SHOULDER, 0)
        for angle, expected in zip(q, [0.0, 90.0, 0.0, 0.0]):
            self.assertAlmostEqual(angle, expected, places=1)

    def test_02(self):
        q = self.kin.ik(Kinematics.L_SEGMENT, 0, Kinematics.Z_SHOULDER - Kinematics.L_SEGMENT - Kinematics.L_GRIPPER, 90)
        for angle, expected in zip(q, [0.0, 90.0, 90.0, 0.0]):
            self.assertAlmostEqual(angle, expected, places=1)

    def test_03(self):
        q = self.kin.ik(Kinematics.L_SEGMENT, 0, 0, 90)
    ...
    def test_06(self):
        q = self.kin.ik(Kinematics.L_SEGMENT + Kinematics.L_GRIPPER, Kinematics.L_SEGMENT, Kinematics.Z_SHOULDER, 0)
